...
class StreamController {
  ...
  generateAdaptationSetVideoOrAudioSegmentRequest(
    ...
    for (let k = 0; k < AdaptationSet['Representation_asArray'].length; k++) {
      ...
      res[Representation.resolvePower].push(
        this.URLUtils.resolve(url, Representation.initializationURL)
      )
      res[Representation.resolvePower].push(
        Representation.mediaURL.map((item) => {
  ...
  getNumberOfMediaSegmentForPeriod(streamId: number): number {
    return this.segmentRequestStruct.request[this.streamId].VideoSegmentRequest[0].video[
      this.videoResolvePower
    ][1].length
  }
  ...
  async startStream(Mpd: Mpd) {
    ...
    let ires = await this.loadInitialSegment()
    ...
    for (
      let i = 0;
      i < (number >= this.firstRequestNumber ? this.firstRequestNumber : number);
      i++
    ) {
  ...
  async onSegmentConsumed(range: VideoBuffers) {
    ...
    if (this.segmentRequestStruct.request[this.streamId] === undefined) {
      ...
      let time = this.timeRangeUtils.getOffestTimeOfMediaSegment(
        this.streamId,
        this.mediaId,
        this.Mpd
      )
  ...
  loadInitialSegment() {
    ...
    return this.loadSegment(
      videoRequest[this.videoResolvePower][0],
      audioRequest[this.audioResolvePower][0]
    )
  }
  ...
  loadMediaSegment() {
    ...
    let videoRequest = stream.VideoSegmentRequest[0].video
    return this.loadSegment(
      videoRequest[this.videoResolvePower][1][this.mediaId],
      audioRequest[this.audioResolvePower][1][this.mediaId]
    )
  }

  async loadSegment(videoURL, audioURL) {
    let p1 = this.urlLoader.load(
      { url: videoURL, responseType: 'arraybuffer' },
      'Segment'
    ) as Promise<any>
    let p2 = this.urlLoader.load(
      { url: audioURL, responseType: 'arraybuffer' },
      'Segment'
    ) as Promise<any>
